...
// Class already selected, displaying loadouts
	...
	switch (_context) do{
		case "weapons" : {
			...
			_weaponstring = _weaponstring + "<br/><br/><br/>Weapons in Backpack:<br/>";
			{
				...
				_weaponstring = _weaponstring + format["<br/>%1  <img image='%3' />    %2",_x select 1,_displname,_picture];
			} foreach (currentpreset select 7);
			_ctrl ctrlSetStructuredText parseText format["%1",_weaponstring];


			// weapon preview (the two pictures)
			_weaponstring = "";
			{
				_config = "CfgWeapons";
				if(not isText (configFile >> _config >> _x >> "displayName")) then {_config = "CfgVehicles"};
