...
_randDir = _randDir + random(10);
_randDist = (random 100) + 500;
_spawnLoc =	[(getPos vehicle _sourcePoint select 0) + (_randDist * sin(_randDir)), (getPos vehicle _sourcePoint select 1) + (_randDist * cos(_randDir)), 500];

missileStart setPos _spawnLoc;
sleep .03;
"missileStart" setMarkerPos _spawnLoc;
sleep .03;

if (isNull _primarytarget) exitWith {
	hintSilent "No Target Found!";
	sleep 1;
	hintSilent "";
	};
