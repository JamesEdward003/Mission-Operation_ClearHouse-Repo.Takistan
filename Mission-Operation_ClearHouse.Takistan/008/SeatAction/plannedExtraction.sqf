...
onMapSingleClick "";
	...
	createMarkerLocal [_hEnd, infTarget];
	_hEnd setMarkerTypeLocal "b_air";
	_hEnd setMarkerShapeLocal "Icon";
	_hEnd setMarkerTextLocal _hEnd;
	_hEnd setMarkerSizeLocal [1,1];
	_hEnd setMarkerColorLocal _lzMkrColor;
...
//	[this] execVM '008\b2_heliDoors.sqf';
//	(driver this) addEventHandler [""handleDamage"", { (damage (_this select 0)) +  0.01; (damage (_this select 2)) +  0.01 }];
//	(gunner this) addEventHandler [""handleDamage"", { (damage (_this select 0)) +  0.01; (damage (_this select 2)) +  0.01 }];